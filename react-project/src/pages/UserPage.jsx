    ...
    >
      ...
      >
        ...
        <div style={{ textAlign: "center", width: "100%" }}>
          ...
          <p style={{ fontSize: "1.5em", marginBottom: "10px" }}>
            수면 시간: {convertToGalaxyAlarmFormat(`${sleepTime}:00`)} 시간
          </p>
          <input
            type="range"
            min="4"
      ...
      <div style={{ width: "33%", marginLeft: "20px" }}>
        {" "}
        {/* 좌측 정보창과 간격을 두기 위해 marginLeft 추가 */}
        <h1>사고 이력</h1>
        <ul>
          {userAccidents.map((accident, index) => (
            <li
              key={index}
              className={`${
                accident.acc_status === "Y" ? "admin-modal-item-hl" : ""
              }`}
            ...
            >
              <div style={{ display: "flex", alignItems: "center" }}>
                <div style={{ width: "350px", marginRight: "20px" }}>
                  <p>
                    발생 시간: {new Date(accident.acc_time).toLocaleString()}
                  </p>
                  <p>사고 정보: {accident.acc_info}</p>
                </div>
                <div>
                  {isEditing.editing &&
                  isEditing.acc_idx === accident.acc_idx ? (
                    <div style={{ display: "flex", alignItems: "center" }}>
                      <input
                        type="text"
